...
function _rptActive(){
  const a={
    ...
    canopy:_rptOn('acc-canopy-sw')||(typeof _canopyPct==='number'&&_canopyPct!=null),
    lst:_rptOn('acc-lst-sw')||(typeof _lstMean==='number'&&_lstMean!=null),
...
async function exportReportPDF(){
  ...
  try{
    ...
    if(a.zoning){
      ...
      const zonesK=(window._rptZones||[]).filter(z=>z.k1!=null||z.k2!=null);
      if(zonesK.length){
        ...
        // per-zone table: Zone | Share | Area | K1 | K2 | Footprint | Floor area | Height
        const cols=[[M,'Zone',52],[M+52,'Share',16],[M+68,'Area m²',24],[M+92,'K1',12],[M+104,'K2',12],[M+116,'Footprint',26],[M+142,'Floor area',26],[M+168,'Height']];
        ensure(6);doc.setFontSize(6.8);doc.setTextColor(130);setFor('Zone');
        ...
        for(const z of zonesK){
          ...
          if(fp)totFp+=fp; if(fa)totFa+=fa;
          doc.setFontSize(7.2);doc.setTextColor(40);setFor(z.kve_zona||'');
          const cells=[z.kve_zona||'—',(z.pct!=null?z.pct+'%':'—'),Math.round(z.area||0).toLocaleString(),
            z.k1!=null?z.k1.toFixed(2):'—',z.k2!=null?z.k2.toFixed(2):'—',
            fp!=null?fp.toLocaleString()+' m²':'—',fa!=null?fa.toLocaleString()+' m²':'—',hgt!=null?hgt+' fl':'—'];
          cols.forEach(([cx],i)=>{setFor(cells[i]);doc.text(String(cells[i]),cx,y);});
          y+=4.6;
        }
        doc.setDrawColor(215);doc.line(M,y,PW-M,y);y+=3.6;
        doc.setFontSize(7.2);doc.setTextColor(20);setFor('Total');
        doc.text('Total',M,y);
        if(totFp)doc.text(totFp.toLocaleString()+' m²',M+116,y);
        if(totFa)doc.text(totFa.toLocaleString()+' m²',M+142,y);
        if(totFp&&totFa)doc.text(Math.floor(totFa/totFp)+' fl',M+168,y);
        y+=6.5;
