...
async function _histCaptureMapImage(){
  ...
  ctxFont(0);
  const wMax=Math.max(ctx.measureText(foot).width,ctx.measureText(title).width,...rows.map(r=>ctx.measureText(r[1]).width+18*s));
  const bw=wMax+pad*2+6*s,bh=pad*2+lh*(rows.length+2);
  const bx=W-bw-16*s,by=H-bh-16*s;
  ctx.fillStyle='rgba(6,6,8,0.88)';
  ctx.beginPath();
  const r0=8*s;
  ctx.moveTo(bx+r0,by);ctx.arcTo(bx+bw,by,bx+bw,by+bh,r0);ctx.arcTo(bx+bw,by+bh,bx,by+bh,r0);ctx.arcTo(bx,by+bh,bx,by,r0);ctx.arcTo(bx,by,bx+bw,by,r0);
  ctx.fill();
